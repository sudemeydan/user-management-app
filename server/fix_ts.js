...
code = code.replace(/const (\w+) = async \(([^)]+)\) =>/g, (match, name, params) => {
    const typedParams = params.split(',').map(p => {
        let pName = p.trim();
        if (!pName) return '';
        if(pName.includes('=')) return pName; // already has default, inferred
        if(pName.includes(':')) return pName; // already typed
        if(pName.startsWith('{')) return pName + ': any'; // destructuring
        return pName + ': any';
    }).filter(p => p !== '').join(', ');
