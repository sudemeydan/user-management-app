...
const createJobPosting = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!jobText && !url) {
      res.status(400).json({ success: false, message: "Ä°ÅŸ ilanÄ± metni veya URL gereklidir." });
      return;
    }
    const jobPosting = await tailoringService.createJobPosting(url, jobText, req.body.role);
    res.json({ success: true, message: "Ä°ÅŸ ilanÄ± baÅŸarÄ±yla kaydedildi!", data: jobPosting });
  } catch (error) {
    next(error);
  }
};
...
const createTailoredCV = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    res.json({ success: true, message: "UyarlanmÄ±ÅŸ CV baÅŸarÄ±yla oluÅŸturuldu!", data: tailoredCV });
  } catch (error) {
    next(error);
  }
};
...
const optimizeTailoredCV = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    res.json({ success: true, message: "UyarlanmÄ±ÅŸ CV PDF'i oluÅŸturuldu!", data: result });
  } catch (error) {
    next(error);
  }
};
